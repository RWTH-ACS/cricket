...
static int hainfo_getserverindex(void *server_ptr)
{
    ...
    for (i = 0; i < hainfo_cnt; ++i) {
        if (hainfo[i].server_ptr != 0 && hainfo[i].server_ptr == server_ptr) {
...
int server_runtime_checkpoint(const char *path, int dump_memory,
...
int server_runtime_restore(const char *path)
{
    ...
    if (cr_restore(path, &rm_memory, &rm_streams, &rm_events, &rm_arrays,
                   cusolver_get_rm(), cublas_get_rm(), &rm_modules,
                   &rm_devices) != 0) {
 ...
 *
 */
bool_t cuda_register_function_1_svc(ptr fatCubinHandle, ptr hostFun,
                                    char *deviceFun, char *deviceName,
                                    int thread_limit, int *result,
                                    struct svc_req *rqstp)
{
    LOGE(LOG_DEBUG, "cudaRegisterFunction(%p, %p, %s, %s, %d)", fatCubinHandle,
    ...
    LOGE(LOG_DEBUG, "added kernel %p->%p to resource manager", hostFun,
         serverFun);
    // __cudaRegisterFunction(&fatCubinHandle, hostFun, deviceFun,
    //                         deviceName, thread_limit, &tid, &bid, &bDim,
    //                         &gDim, &wSize);

    // LOGE(LOG_DEBUG, "-> %p, {%d, %d, %d}, {%d, %d, %d}, {%d, %d, %d}, {%d,
    // %d, %d}, %d)",
    //                 fatCubinHandle,
    //                 tid.x, tid.y, tid.z,
    //                 bid.x, bid.y, bid.z,
...
/* ############### RUNTIME API ############### */
/* ### Device Management ### */
bool_t cuda_choose_device_1_svc(mem_data prop, int_result *result,
                                struct svc_req *rqstp)
{
    ...
    if (prop.mem_data_len != sizeof(struct cudaDeviceProp)) {
        LOGE(LOG_ERROR,
             "Received wrong amount of data: expected %zu but got %zu",
             sizeof(struct cudaDeviceProp), prop.mem_data_len);
        return 0;
    }
...
bool_t cuda_device_get_attribute_1_svc(int attr, int device, int_result *result,
                                       struct svc_req *rqstp)
{
    LOGE(LOG_DEBUG, "cudaDeviceGetAttribute");
    result->err = cudaDeviceGetAttribute(&result->int_result_u.data,
                                         (enum cudaDeviceAttr)attr, device);
    return 1;
}

bool_t cuda_device_get_by_pci_bus_id_1_svc(char *pciBusId, int_result *result,
                                           struct svc_req *rqstp)
{
...
bool_t cuda_device_get_cache_config_1_svc(int_result *result,
                                          struct svc_req *rqstp)
{
...
bool_t cuda_device_get_limit_1_svc(int limit, u64_result *result,
                                   struct svc_req *rqstp)
{
...
//        /*mem_result CUDA_DEVICE_GET_NVSCISYNC_ATTRIBUTES(int ,int)     =
//        106;*/

bool_t cuda_device_get_p2p_attribute_1_svc(int attr, int srcDevice,
                                           int dstDevice, int_result *result,
                                           struct svc_req *rqstp)
{
...
bool_t cuda_device_get_pci_bus_id_1_svc(int len, int device, str_result *result,
                                        struct svc_req *rqstp)
{
...
bool_t cuda_device_get_shared_mem_config_1_svc(int_result *result,
                                               struct svc_req *rqstp)
{
    LOGE(LOG_DEBUG, "cudaDeviceGetSharedMemConfig");
    result->err = cudaDeviceGetSharedMemConfig(
        (enum cudaSharedMemConfig *)&result->int_result_u.data);
    return 1;
}

bool_t cuda_device_get_stream_priority_range_1_svc(dint_result *result,
                                                   struct svc_req *rqstp)
{
    LOGE(LOG_DEBUG, "cudaDeviceGetStreamPriorityRange");
    result->err = cudaDeviceGetStreamPriorityRange(
        &result->dint_result_u.data.i1, &result->dint_result_u.data.i2);
    return 1;
}

bool_t cuda_device_get_texture_lmw_1_svc(cuda_channel_format_desc fmtDesc,
                                         int device, u64_result *result,
                                         struct svc_req *rqstp)
{
    ...
    result->err = cudaDeviceGetTexture1DLinearMaxWidth(
        &result->u64_result_u.u64, &desc, device);
    return 1;
#else
...
bool_t cuda_device_set_cache_config_1_svc(int cacheConfig, int *result,
                                          struct svc_req *rqstp)
{
    RECORD_API(int);
...
bool_t cuda_device_set_limit_1_svc(int limit, size_t value, int *result,
                                   struct svc_req *rqstp)
{
...
bool_t cuda_device_set_shared_mem_config_1_svc(int config, int *result,
                                               struct svc_req *rqstp)
{
    RECORD_API(int);
...
bool_t cuda_get_device_properties_1_svc(int device,
                                        cuda_device_prop_result *result,
                                        struct svc_req *rqstp)
{
    LOGE(LOG_DEBUG, "cudaGetDeviceProperties");
    if (sizeof(result->cuda_device_prop_result_u.data) !=
        sizeof(struct cudaDeviceProp)) {
        LOGE(LOG_ERROR,
             "cuda_device_prop_result size mismatch, result %d prop %d",
             sizeof(result->cuda_device_prop_result_u.data),
             sizeof(struct cudaDeviceProp));
        return 0;
    }
    result->err = cudaGetDeviceProperties(
        (void *)result->cuda_device_prop_result_u.data, device);
    return 1;
}

//        /*int        CUDA_IPC_CLOSE_MEM_HANDLE(ptr)                     =
//        121;*/
//        /*ptr_result CUDA_IPC_GET_EVENT_HANDLE(int)                     =
//        122;*/
//        /*ptr_result CUDA_IPC_GET_MEM_HANDLE(ptr)                       =
//        123;*/
//        /*ptr_result CUDA_IPC_OPEN_EVENT_HANDLE(ptr)                    =
//        124;*/
//        /*ptr_result CUDA_IPC_OPEN_MEM_HANDLE(ptr, int)                 =
//        125;*/
...
bool_t cuda_set_device_flags_1_svc(int flags, int *result,
                                   struct svc_req *rqstp)
{
    RECORD_API(int);
...
bool_t cuda_set_valid_devices_1_svc(mem_data device_arr, int len, int *result,
                                    struct svc_req *rqstp)
{
    ...
    if (device_arr.mem_data_len != len * sizeof(int)) {
        LOGE(LOG_ERROR,
             "mismatch between expected size (%d) and received size (%d)",
             len * sizeof(int), device_arr.mem_data_len);
        return 0;
    }
...
bool_t cuda_get_error_name_1_svc(int error, str_result *result,
                                 struct svc_req *rqstp)
{
    const char *str;
...
bool_t cuda_get_error_string_1_svc(int error, str_result *result,
                                   struct svc_req *rqstp)
{
    const char *str;
    result->str_result_u.str = malloc(128);
...
bool_t cuda_ctx_reset_persisting_l2cache_1_svc(int *result,
                                               struct svc_req *rqstp)
{
#if CUDART_VERSION >= 11000
    RECORD_VOID_API;
...
bool_t cuda_stream_copy_attributes_1_svc(ptr dst, ptr src, int *result,
                                         struct svc_req *rqstp)
{
#if CUDART_VERSION >= 11000
    ...
    *result =
        cudaStreamCopyAttributes(resource_mg_get(&rm_streams, (void *)dst),
...
bool_t cuda_stream_create_1_svc(ptr_result *result, struct svc_req *rqstp)
{
    ...
    if (resource_mg_create(&rm_streams, (void *)result->ptr_result_u.ptr) !=
        0) {
...
bool_t cuda_stream_create_with_flags_1_svc(int flags, ptr_result *result,
                                           struct svc_req *rqstp)
{
    RECORD_API(int);
    RECORD_SINGLE_ARG(flags);
    ...
    result->err =
        cudaStreamCreateWithFlags((void *)&result->ptr_result_u.ptr, flags);
    if (resource_mg_create(&rm_streams, (void *)result->ptr_result_u.ptr) !=
        0) {
        LOGE(LOG_ERROR, "error in resource manager");
    }
...
bool_t cuda_stream_create_with_priority_1_svc(int flags, int priority,
                                              ptr_result *result,
                                              struct svc_req *rqstp)
{
    ...
    result->err = cudaStreamCreateWithPriority(
        (void *)&result->ptr_result_u.ptr, flags, priority);
    if (resource_mg_create(&rm_streams, (void *)result->ptr_result_u.ptr) !=
        0) {
        LOGE(LOG_ERROR, "error in resource manager");
    }
...
bool_t cuda_stream_end_capture_1_svc(ptr stream, ptr_result *result,
                                     struct svc_req *rqstp)
{
    RECORD_API(ptr);
    RECORD_SINGLE_ARG(stream);
    ...
    result->err = cudaStreamEndCapture(
        stream_ptr, (cudaGraph_t *)&result->ptr_result_u.ptr);

    if (result->err == cudaSuccess) {
        if (resource_mg_create(&rm_graphs, (void *)result->ptr_result_u.ptr) !=
            0) {
            LOGE(LOG_ERROR, "error in resource manager");
        }
    }
...
bool_t cuda_graph_instantiate_1_svc(ptr graph, size_t flags, ptr_result *result,
                                    struct svc_req *rqstp)
{
    ...
    result->err = cudaGraphInstantiate(
        (cudaGraphExec_t *)&result->ptr_result_u.ptr, graph_ptr, flags);
                                  ...
                                  struct svc_req *rqstp)
{
    ...
    LOGE(LOG_DEBUG, "cudaGraphGetNodes(%p, %d, %d)", graph, nodes_is_null,
         num_nodes);
    cudaGraph_t graph_ptr = resource_mg_get(&rm_graphs, (void *)graph);
    cudaGraphNode_t *nodes = NULL;
    if (!nodes_is_null) {
        result->mem_result_u.data.mem_data_len =
            sizeof(size_t) + num_nodes * sizeof(cudaGraphNode_t);
        result->mem_result_u.data.mem_data_val =
            malloc(sizeof(size_t) + num_nodes * sizeof(cudaGraphNode_t));
        if (result->mem_result_u.data.mem_data_val == NULL) {
            LOGE(LOG_ERROR, "malloc failed");
            return 0;
        }
        nodes = (cudaGraphNode_t *)(result->mem_result_u.data.mem_data_val +
                                    sizeof(size_t));
    ...
    LOGE(LOG_DEBUG, "cudaGraphGetNodes: numNodes: %d, return: %d",
         *num_nodes_ptr, result->err);
    RECORD_RESULT(integer, result->err);
    return 1;
}

/* What datatypes are in the union cudaStreamAttrValue? */
//        /* ?         CUDA_STREAM_GET_ATTRIBUTE(ptr, int)                =
//        260;*/
/* Capture API does not make sense without graph API */
//        /* ?         CUDA_STREAM_GET_CAPTURE_INFO(ptr)                  =
//        261;*/


bool_t cuda_stream_get_flags_1_svc(ptr hStream, int_result *result,
                                   struct svc_req *rqstp)
{
    LOGE(LOG_DEBUG, "cudaStreamGetFlags");
    result->err =
        cudaStreamGetFlags(resource_mg_get(&rm_streams, (void *)hStream),
                           (unsigned *)&result->int_result_u.data);
    return 1;
}

bool_t cuda_stream_get_priority_1_svc(ptr hStream, int_result *result,
                                      struct svc_req *rqstp)
{
    LOGE(LOG_DEBUG, "cudaStreamGetPriority");
    result->err =
        cudaStreamGetPriority(resource_mg_get(&rm_streams, (void *)hStream),
                              &result->int_result_u.data);
    return 1;
}

bool_t cuda_stream_is_capturing_1_svc(ptr stream, int_result *result,
                                      struct svc_req *rqstp)
{
...
bool_t cuda_stream_query_1_svc(ptr hStream, int *result, struct svc_req *rqstp)
{
    ...
    *result = cudaStreamQuery(resource_mg_get(&rm_streams, (void *)hStream));
    return 1;
}

/* What datatypes are in the union cudaStreamAttrValue? */
//        /*int          CUDA_STREAM_SET_ATTRIBUTE(ptr, int, ?)             =
//        266;*/

bool_t cuda_stream_synchronize_1_svc(ptr stream, int *result,
                                     struct svc_req *rqstp)
{
    // RECORD_API(uint64_t);
    // RECORD_SINGLE_ARG(stream);
    ...
    *result =
        cudaStreamSynchronize(resource_mg_get(&rm_streams, (void *)stream));
    // RECORD_RESULT(integer, *result);
    return 1;
}

bool_t cuda_stream_wait_event_1_svc(ptr stream, ptr event, int flags,
                                    int *result, struct svc_req *rqstp)
{
    ...
    *result =
        cudaStreamWaitEvent(resource_mg_get(&rm_streams, (void *)stream),
                            resource_mg_get(&rm_events, (void *)event), flags);
    RECORD_RESULT(integer, *result);
    return 1;
}

bool_t cuda_thread_exchange_stream_capture_mode_1_svc(int mode,
                                                      int_result *result,
                                                      struct svc_req *rqstp)
{
    RECORD_API(int);
    ...
    result->err =
        cudaThreadExchangeStreamCaptureMode((void *)&result->int_result_u.data);
    RECORD_RESULT(integer, result->int_result_u.data);
    return 1;
}

bool_t cuda_stream_begin_capture_1_svc(ptr stream, int mode, int *result,
                                       struct svc_req *rqstp)
{
    ...
    *result = cudaStreamBeginCapture(
        resource_mg_get(&rm_streams, (void *)stream), mode);
    RECORD_RESULT(integer, *result);
    return 1;
}
...
bool_t cuda_event_create_1_svc(ptr_result *result, struct svc_req *rqstp)
{
    ...
    result->err =
        cudaEventCreate((struct CUevent_st **)&result->ptr_result_u.ptr);
...
bool_t cuda_event_create_with_flags_1_svc(int flags, ptr_result *result,
                                          struct svc_req *rqstp)
{
    RECORD_API(int);
    RECORD_SINGLE_ARG(flags);
    LOGE(LOG_DEBUG, "cudaEventCreateWithFlags");
    result->err = cudaEventCreateWithFlags(
        (struct CUevent_st **)&result->ptr_result_u.ptr, flags);
    if (resource_mg_create(&rm_events, (void *)result->ptr_result_u.ptr) != 0) {
        LOGE(LOG_ERROR, "error in resource manager");
    }
    RECORD_RESULT(ptr_result_u, *result);
...
bool_t cuda_event_destroy_1_svc(ptr event, int *result, struct svc_req *rqstp)
{
    ...
    *result = cudaEventDestroy(resource_mg_get(&rm_events, (void *)event));
    RECORD_RESULT(integer, *result);
    return 1;
}

bool_t cuda_event_elapsed_time_1_svc(ptr start, ptr end, float_result *result,
                                     struct svc_req *rqstp)
{
    LOGE(LOG_DEBUG, "cudaEventElapsedTime");
    result->err =
        cudaEventElapsedTime(&result->float_result_u.data,
...
bool_t cuda_event_query_1_svc(ptr event, int *result, struct svc_req *rqstp)
{
    ...
    *result = cudaEventQuery(resource_mg_get(&rm_events, (void *)event));
    RECORD_RESULT(integer, *result);
    return 1;
}

bool_t cuda_event_record_1_svc(ptr event, ptr stream, int *result,
                               struct svc_req *rqstp)
{
    ...
    *result = cudaEventRecord(resource_mg_get(&rm_events, (void *)event),
                              resource_mg_get(&rm_streams, (void *)stream));
    RECORD_RESULT(integer, *result);
    return 1;
}

bool_t cuda_event_record_with_flags_1_svc(ptr event, ptr stream, int flags,
                                          int *result, struct svc_req *rqstp)
{
#if CUDART_VERSION >= 11000
    ...
    *result =
        cudaEventRecordWithFlags(resource_mg_get(&rm_events, (void *)event),
                                 resource_mg_get(&rm_streams, (void *)stream),
                                 flags);
    RECORD_RESULT(integer, *result);
    return 1;
...
bool_t cuda_event_synchronize_1_svc(ptr event, int *result,
                                    struct svc_req *rqstp)
{
    RECORD_API(ptr);
    RECORD_SINGLE_ARG(event);
    LOGE(LOG_DEBUG, "cudaEventSynchronize");
    *result = cudaEventSynchronize(resource_mg_get(&rm_events, (void *)event));
    RECORD_RESULT(integer, *result);
    return 1;
}
...
bool_t cuda_func_get_attributes_1_svc(ptr func, mem_result *result,
                                      struct svc_req *rqstp)
{
    LOGE(LOG_DEBUG, "cudaFuncGetAttributes");
    LOGE(LOG_WARNING, "cudaFuncGetAttributes is probably not working "
                      "correctly.");
    result->mem_result_u.data.mem_data_val =
...
bool_t cuda_func_set_attributes_1_svc(ptr func, int attr, int value,
                                      int *result, struct svc_req *rqstp)
{
    ...
    *result = cuFuncSetAttribute(
        (CUfunction)resource_mg_get(&rm_functions, (void *)func), attr, value);
    RECORD_RESULT(integer, *result);
    return 1;
}

bool_t cuda_func_set_cache_config_1_svc(ptr func, int cacheConfig, int *result,
                                        struct svc_req *rqstp)
{
    ...
    LOGE(LOG_DEBUG, "cudaFuncSetCacheConfig");
    *result = cuFuncSetCacheConfig(
        (CUfunction)resource_mg_get(&rm_functions, (void *)func), cacheConfig);
    RECORD_RESULT(integer, *result);
    return 1;
}

bool_t cuda_func_set_shared_mem_config_1_svc(ptr func, int config, int *result,
                                             struct svc_req *rqstp)
{
    ...
    RECORD_ARG(2, config);
    LOGE(LOG_DEBUG, "cudaFuncSetSharedMemConfig");
    *result = cuFuncSetSharedMemConfig(
        (CUfunction)resource_mg_get(&rm_functions, (void *)func), config);
    RECORD_RESULT(integer, *result);
    return 1;
}

bool_t cuda_launch_cooperative_kernel_1_svc(ptr func, rpc_dim3 gridDim,
                                            rpc_dim3 blockDim, mem_data args,
                                            size_t sharedMem, ptr stream,
                                            int *result, struct svc_req *rqstp)
{
    ...
    LOGE(LOG_DEBUG,
         "cudaLaunchCooperativeKernel(func=%p, gridDim=[%d,%d,%d], "
         "blockDim=[%d,%d,%d], args=%p, sharedMem=%d, stream=%p)",
         func, cuda_gridDim.x, cuda_gridDim.y, cuda_gridDim.z, cuda_blockDim.x,
         cuda_blockDim.y, cuda_blockDim.z, cuda_args, sharedMem,
         (void *)stream);

    *result = cudaLaunchCooperativeKernel(
        resource_mg_get(&rm_kernels, (void *)func), cuda_gridDim, cuda_blockDim,
        cuda_args, sharedMem, resource_mg_get(&rm_streams, (void *)stream));
    RECORD_RESULT(integer, *result);
 ...
 * __host__ cudaError_t cudaLaunchHostFunc ( cudaStream_t stream, cudaHostFn_t
 * fn, void* userData ) Enqueues a host function call in a stream.
 */
                                ...
                                int *result, struct svc_req *rqstp)
{
    ...
    LOGE(LOG_DEBUG,
         "cudaLaunchKernel(func=%p, gridDim=[%d,%d,%d], blockDim=[%d,%d,%d], "
         "args=%p, sharedMem=%d, stream=%p)",
         resource_mg_get(&rm_functions, (void *)func), cuda_gridDim.x,
         cuda_gridDim.y, cuda_gridDim.z, cuda_blockDim.x, cuda_blockDim.y,
         cuda_blockDim.z, cuda_args, sharedMem, (void *)stream);

    *result = cuLaunchKernel(
        (CUfunction)resource_mg_get(&rm_functions, (void *)func), gridDim.x,
        gridDim.y, gridDim.z, blockDim.x, blockDim.y, blockDim.z, sharedMem,
        resource_mg_get(&rm_streams, (void *)stream), cuda_args, NULL);
...
bool_t cuda_occupancy_available_dsmpb_1_svc(ptr func, int numBlocks,
                                            int blockSize, u64_result *result,
                                            struct svc_req *rqstp)
{
#if CUDART_VERSION >= 11000
...
bool_t cuda_occupancy_max_active_bpm_1_svc(ptr func, int blockSize,
...
bool_t cuda_occupancy_max_active_bpm_with_flags_1_svc(ptr func, int blockSize,
                                                      size_t dynamicSMemSize,
                                                      int flags,
                                                      int_result *result,
                                                      struct svc_req *rqstp)
...
bool_t cuda_array_get_info_1_svc(ptr array, mem_result *result,
                                 struct svc_req *rqstp)
{
    LOGE(LOG_DEBUG, "cudaArrayGetInfo");
    result->mem_result_u.data.mem_data_len =
        sizeof(struct cudaChannelFormatDesc) + sizeof(struct cudaExtent) +
        sizeof(int);
    result->mem_result_u.data.mem_data_val =
        malloc(sizeof(struct cudaChannelFormatDesc) +
               sizeof(struct cudaExtent) + sizeof(int));
    struct cudaChannelFormatDesc *desc =
    ...
    result->err = cudaArrayGetInfo(desc, extent, flags,
                                   resource_mg_get(&rm_arrays, (void *)array));
    return 1;
}

bool_t cuda_array_get_sparse_properties_1_svc(ptr array, mem_result *result,
                                              struct svc_req *rqstp)
{
#if CUDART_VERSION >= 11000
    ...
    result->mem_result_u.data.mem_data_val =
        malloc(sizeof(struct cudaArraySparseProperties));
    struct cudaArraySparseProperties *sparseProperties =
        (void *)result->mem_result_u.data.mem_data_val;

    result->err = cudaArrayGetSparseProperties(
        sparseProperties, resource_mg_get(&rm_arrays, (void *)array));
    return 1;
#else
    LOGE(LOG_ERROR, "not compiled with CUDA 11 support");
    return 1;
...
bool_t cuda_free_1_svc(ptr devPtr, int *result, struct svc_req *rqstp)
{
    ...
    *result = cudaFree(memory_mg_get(&rm_memory, (void *)devPtr));
    ...
    //         arg =
    //         ((cuda_memcpy_to_symbol_shm_1_argument*)r->arguments)->arg2;
    //         break;
    //     case CUDA_MEMCPY_IB:
    ...
    // }
    // if (ret != 0) {
    //     LOGE(LOG_ERROR, "could not find a malloc call associated with this
    //     free call"); *result = CUDA_ERROR_UNKNOWN;
    // }
    RECORD_RESULT(integer, *result);
    return 1;
}
...
bool_t cuda_free_host_1_svc(int index, int *result, struct svc_req *rqstp)
{
    ...
    *result = cudaErrorInitializationError;
    ...
    if (hainfo[index].idx != 0 && hainfo[index].idx == index) {
...
/* cudaFreeMipmappedArray ( cudaMipmappedArray_t mipmappedArray ) is not
 * implemented */
/* cudaGetMipmappedArrayLevel ( cudaArray_t* levelArray,
 * cudaMipmappedArray_const_t mipmappedArray, unsigned int  level ) is not
 * implemented */


bool_t cuda_get_symbol_address_1_svc(ptr symbol, ptr_result *result,
                                     struct svc_req *rqstp)
{
    ...
    result->err = cudaGetSymbolAddress((void **)&result->ptr_result_u.ptr,
                                       (void *)symbol);
    return 1;
}

bool_t cuda_get_symbol_size_1_svc(ptr symbol, u64_result *result,
                                  struct svc_req *rqstp)
{
    // Symbol is in host address space
...
bool_t cuda_host_alloc_1_svc(size_t size, unsigned int flags, sz_result *result,
                             struct svc_req *rqstp)
{
    ...
    if (socktype == UNIX ||
        (shm_enabled && cpu_utils_is_local_connection(rqstp))) { // Use local
                                                                 // shared
                                                                 // memory
        ...
        if ((fd_shm = shm_open(shm_name, O_RDWR | O_CREAT | O_TRUNC,
                               S_IRWXU)) == -1) {
            LOGE(LOG_ERROR,
                 "could not open shared memory \"%s\" with size %d: %s",
                 shm_name, size, strerror(errno));
            goto out;
        }
        ...
        LOGE(LOG_DEBUG, "shm opened with name \"%s\", size: %d", shm_name,
             size);
        if ((shm_addr = mmap(NULL, size, PROT_READ | PROT_WRITE, MAP_SHARED,
                             fd_shm, 0)) == MAP_FAILED) {
        ...
        if ((result->err = cudaHostRegister(shm_addr, size, flags)) !=
            cudaSuccess) {
        ...
        LOGE(LOG_ERROR, "cudaHostAlloc is not supported for other transports "
                        "than UNIX. This error means cricket_server and "
                        "cricket_client are not compiled correctly (different "
                        "transports)");
        goto out;
    }
...
bool_t cuda_host_alloc_regshm_1_svc(size_t hainfo_idx, ptr client_ptr,
                                    int *result, struct svc_req *rqstp)
{
    char *shm_name = NULL;
    ...
    *result = cudaErrorMemoryAllocation;
    ...
    if (socktype != UNIX &&
        !(shm_enabled && cpu_utils_is_local_connection(rqstp))) {
        LOGE(LOG_ERROR, "cudaHostAllocRegShm is only supported for local "
                        "connections.");
        goto out;
    }
...
bool_t cuda_host_get_device_pointer_1_svc(ptr pHost, int flags,
                                          ptr_result *result,
                                          struct svc_req *rqstp)
{
    LOGE(LOG_DEBUG, "cudaHostGetDevicePointer");
    // TODO: implement correctly using the resouce manager
    // result->err = cudaHostGetDevicePointer((void**)&result->ptr_result_u.ptr,
    // (void*)pHost, flags);
    result->err = cudaErrorUnknown;
    return 1;
}

bool_t cuda_host_get_flags_1_svc(ptr pHost, int_result *result,
                                 struct svc_req *rqstp)
{
...
/* cudaHostRegister ( void* ptr, size_t size, unsigned int  flags ) not possible
 * using shared memory maybe we can register infiniband segment though?
 */
...
#ifdef WITH_IB
    result->err = ib_allocate_memreg((void **)&result->ptr_result_u.ptr, argp,
                                     hainfo_cnt, true);
    if (result->err == 0) {
        hainfo[hainfo_cnt].idx = hainfo_cnt;
...
bool_t cuda_malloc_3d_1_svc(size_t depth, size_t height, size_t width,
                            pptr_result *result, struct svc_req *rqstp)
{
...
bool_t cuda_malloc_3d_array_1_svc(cuda_channel_format_desc desc, size_t depth,
                                  size_t height, size_t width, int flags,
                                  ptr_result *result, struct svc_req *rqstp)
{
    ...
    struct cudaChannelFormatDesc cuda_desc = {
        .f = desc.f, .w = desc.w, .x = desc.x, .y = desc.y, .z = desc.z
    };
    struct cudaExtent extent = { .depth = depth,
                                 .height = height,
                                 .width = width };
    LOGE(LOG_DEBUG, "cudaMalloc3DArray");
    result->err = cudaMalloc3DArray((void *)&result->ptr_result_u.ptr,
                                    &cuda_desc, extent, flags);
...
bool_t cuda_malloc_array_1_svc(cuda_channel_format_desc desc, size_t width,
                               size_t height, int flags, ptr_result *result,
                               struct svc_req *rqstp)
{
    ...
    result->err = cudaMallocArray((void *)&result->ptr_result_u.ptr, &cuda_desc,
                                  width, height, flags);
    resource_mg_create(&rm_arrays, (void *)result->ptr_result_u.ptr);
 ...
 * cudaMemAttachGlobal ) is not implemented */
/* cudaMallocMipmappedArray ( cudaMipmappedArray_t* mipmappedArray, const
 * cudaChannelFormatDesc* desc, cudaExtent extent, unsigned int  numLevels,
 * unsigned int  flags = 0 ) is not implemented */

bool_t cuda_malloc_pitch_1_svc(size_t width, size_t height,
                               ptrsz_result *result, struct svc_req *rqstp)
{
    ...
    result->err =
        cudaMallocPitch((void *)&result->ptrsz_result_u.data.p,
                        &result->ptrsz_result_u.data.s, width, height);
    memory_mg_create(&rm_memory, (void *)result->ptrsz_result_u.data.p,
                     width * height);
...
bool_t cuda_mem_advise_1_svc(ptr devPtr, size_t count, int advice, int device,
                             int *result, struct svc_req *rqstp)
{
...
bool_t cuda_mem_prefetch_async_1_svc(ptr devPtr, size_t count, int dstDevice,
                                     ptr stream, int *result,
                                     struct svc_req *rqstp)
{
...
/* cuda_mem_range_get_attribute_1_svc(int attribute, ptr devPtr, size_t count,
 * mem_result *result, struct svc_req *rqstp) unsupported because requires
 * unified memors */
/* cudaMemRangeGetAttribute ( void* data, size_t dataSize, cudaMemRangeAttribute
 * attribute, const void* devPtr, size_t count ) unsupported because requires
 * unified memory */

/* CUDA_MEMCPY Family */

bool_t cuda_memcpy_htod_1_svc(uint64_t ptr, mem_data mem, size_t size,
                              int *result, struct svc_req *rqstp)
{
    ...
    LOGE(LOG_DEBUG, "cudaMemcpyHtoD(%p, %p, %zu)", (void *)ptr,
         mem.mem_data_val, size);
    ...
    if ((*result = cudaHostRegister(mem.mem_data_val, size,
                                    cudaHostRegisterMapped)) != cudaSuccess) {
        LOGE(LOG_ERROR, "cudaHostRegister failed: %d.", *result);
        return 1;
    }
#endif
    *result = cudaMemcpy(memory_mg_get(&rm_memory, (void *)ptr),
                         mem.mem_data_val, size, cudaMemcpyHostToDevice);
#ifdef WITH_MEMCPY_REGISTER
...
bool_t cuda_memcpy_mt_htod_1_svc(uint64_t dest, size_t size, int thread_num,
                                 dint_result *result, struct svc_req *rqstp)
{
    ...
    if (mt_memcpy_init_server(server, (void *)dest, size, MT_MEMCPY_HTOD,
                              thread_num) != 0) {
...
bool_t cuda_memcpy_mt_dtoh_1_svc(uint64_t src, size_t size, int thread_num,
                                 dint_result *result, struct svc_req *rqstp)
{
    ...
    if (mt_memcpy_init_server(server, (void *)src, size, MT_MEMCPY_DTOH,
                              thread_num) != 0) {
        LOGE(LOG_ERROR, "mt_memcpy_init_server failed.");
        result->err = cudaErrorUnknown;
        return 1;
    ...
    *result = mt_memcpy_sync_server(server);
    ...
    if (id == mt_memcpy_list.length - 1) {
        ...
        while (mt_memcpy_list.length > 0 &&
               ((mt_memcpy_server_t *)list_get(&mt_memcpy_list,
                                               mt_memcpy_list.length - 1))
                       ->dev_ptr == NULL) {
...
bool_t cuda_memcpy_dtod_1_svc(ptr dst, ptr src, size_t size, int *result,
                              struct svc_req *rqstp)
{
    ...
    LOGE(LOG_DEBUG, "cudaMemcpyDtoD(%p, %p, %zu)", (void *)dst, (void *)src,
         size);
    *result = cudaMemcpy(memory_mg_get(&rm_memory, (void *)dst),
                         memory_mg_get(&rm_memory, (void *)src), size,
...
bool_t cuda_memcpy_ib_1_svc(int index, ptr device_ptr, size_t size, int kind,
                            int *result, struct svc_req *rqstp)
{
    ...
    *result = cudaErrorInitializationError;
    ...
    if (kind == cudaMemcpyHostToDevice) {
        ...
        //        info->device_ptr = memory_mg_get(&rm_memory,
        //        (void*)device_ptr);
        info->size = size;
        info->result = 0;
        ...
        *result = 0;
        ...
        // -> DONE first arg will bei ib registered device mem reg -> ?
    }
out:
    RECORD_RESULT(integer, *result);
    return 1;
...
bool_t cuda_memcpy_shm_1_svc(int index, ptr device_ptr, size_t size, int kind,
                             int *result, struct svc_req *rqstp)
{
    ...
    RECORD_ARG(4, kind);
    LOGE(LOG_DEBUG,
         "cudaMemcpyShm(index: %d, device_ptr: %p, size: %d, kind: %d)", index,
         device_ptr, size, kind);
    *result = cudaErrorInitializationError;
    if (index >= hainfo_cnt || hainfo[index].idx != index) {

        LOGE(LOG_ERROR, "inconsistent state: index: %d, hainfo[index].idx: %d",
             index, hainfo[index].idx);
        goto out;
    }
    if (hainfo[index].size < size) {
    ...
    if (kind == cudaMemcpyHostToDevice) {
        *result = cudaMemcpy(memory_mg_get(&rm_memory, (void *)device_ptr),
                             hainfo[index].server_ptr, size, kind);
    } else if (kind == cudaMemcpyDeviceToHost) {
        *result = cudaMemcpy(hainfo[index].server_ptr,
...
bool_t cuda_memcpy_dtoh_1_svc(uint64_t ptr, size_t size, mem_result *result,
                              struct svc_req *rqstp)
{
    ...
    if ((result->err = cudaHostRegister(result->mem_result_u.data.mem_data_val,
                                        size, cudaHostRegisterMapped)) !=
        cudaSuccess) {
        LOGE(LOG_ERROR, "cudaHostRegister failed.");
        goto out;
    }
#endif
    result->err = cudaMemcpy(result->mem_result_u.data.mem_data_val,
                             memory_mg_get(&rm_memory, (void *)ptr), size,
 ...
 * size_t width, size_t height, cudaMemcpyKind kind ) not implemented yet */
/* cudaMemcpy2DArrayToArray ( cudaArray_t dst, size_t wOffsetDst, size_t
 * hOffsetDst, cudaArray_const_t src, size_t wOffsetSrc, size_t hOffsetSrc,
 * size_t width, size_t height, cudaMemcpyKind kind = cudaMemcpyDeviceToDevice )
 * not implemented yet */
/* cudaMemcpy2DAsync ( void* dst, size_t dpitch, const void* src, size_t spitch,
 * size_t width, size_t height, cudaMemcpyKind kind, cudaStream_t stream = 0 )
 * not implemented yet */
/* cudaMemcpy2DFromArray ( void* dst, size_t dpitch, cudaArray_const_t src,
 * size_t wOffset, size_t hOffset, size_t width, size_t height, cudaMemcpyKind
 * kind ) not implemented yet */
/* cudaMemcpy2DFromArrayAsync ( void* dst, size_t dpitch, cudaArray_const_t src,
 * size_t wOffset, size_t hOffset, size_t width, size_t height, cudaMemcpyKind
 * kind, cudaStream_t stream = 0 ) not implemented yet */
/* cudaMemcpy2DToArray ( cudaArray_t dst, size_t wOffset, size_t hOffset, const
 * void* src, size_t spitch, size_t width, size_t height, cudaMemcpyKind kind )
 * not implemented yet */
/* cudaMemcpy2DToArrayAsync ( cudaArray_t dst, size_t wOffset, size_t hOffset,
 * const void* src, size_t spitch, size_t width, size_t height, cudaMemcpyKind
 * kind, cudaStream_t stream = 0 ) not implemented yet */
/* cudaMemcpy3D ( const cudaMemcpy3DParms* p ) not implemented yet */
/* cudaMemcpy3DAsync ( const cudaMemcpy3DParms* p, cudaStream_t stream = 0 ) not
 * implemented yet */
/* cudaMemcpy3DPeer ( const cudaMemcpy3DPeerParms* p ) not implemented yet */
/* cudaMemcpy3DPeerAsync ( const cudaMemcpy3DPeerParms* p, cudaStream_t stream =
 * 0 ) not implemented yet */

/* More memcpy Family */

/* cudaMemcpyAsync ( void* dst, const void* src, size_t count, cudaMemcpyKind
 * kind, cudaStream_t stream = 0 ) not implemented yet */
/* cudaMemcpyFromSymbol ( void* dst, const void* symbol, size_t count, size_t
 * offset = 0, cudaMemcpyKind kind = cudaMemcpyDeviceToHost ) not implemented
 * yet. see cudaMemcpyToSymbol */
/* cudaMemcpyFromSymbolAsync ( void* dst, const void* symbol, size_t count,
 * size_t offset, cudaMemcpyKind kind, cudaStream_t stream = 0 ) not implemented
 * yet */
/* cudaMemcpyPeer ( void* dst, int  dstDevice, const void* src, int  srcDevice,
 * size_t count ) not implemented yet. see cudaMemcpyDtoD */
/* cudaMemcpyPeerAsync ( void* dst, int  dstDevice, const void* src, int
 * srcDevice, size_t count, cudaStream_t stream = 0 ) */

bool_t cuda_memcpy_to_symbol_1_svc(uint64_t symbolptr, mem_data mem,
                                   size_t size, size_t offset, int *result,
                                   struct svc_req *rqstp)
{
    LOGE(LOG_DEBUG, "cudaMemcpyToSymbol(%p, %p, %zu, %zu)", symbolptr,
         mem.mem_data_val, size, offset);
    ...
    return cuda_memcpy_htod_1_svc((ptr)(symbol_addr + offset), mem, size,
                                  result, rqstp);
}

bool_t cuda_memcpy_to_symbol_shm_1_svc(int index, ptr device_ptr, size_t size,
                                       size_t offset, int kind, int *result,
                                       struct svc_req *rqstp)
{
    ...
    return cuda_memcpy_shm_1_svc(index, (ptr)(symbol_addr + offset), size, kind,
                                 result, rqstp);
}

/* cudaMemcpyToSymbolAsync ( const void* symbol, const void* src, size_t count,
 * size_t offset, cudaMemcpyKind kind, cudaStream_t stream = 0 ) not implemented
 * yet */

/* cudaMemset family */
bool_t cuda_memset_1_svc(ptr devPtr, int value, size_t count, int *result,
                         struct svc_req *rqstp)
{
    ...
    *result =
        cudaMemset(memory_mg_get(&rm_memory, (void *)devPtr), value, count);
    RECORD_RESULT(integer, *result);
    return 1;
}

bool_t cuda_memset_2d_1_svc(ptr devPtr, size_t pitch, int value, size_t width,
                            size_t height, int *result, struct svc_req *rqstp)
{
    ...
    *result = cudaMemset2D(memory_mg_get(&rm_memory, (void *)devPtr), pitch,
                           value, width, height);
    RECORD_RESULT(integer, *result);
    return 1;
}

bool_t cuda_memset_2d_async_1_svc(ptr devPtr, size_t pitch, int value,
                                  size_t width, size_t height, ptr stream,
                                  int *result, struct svc_req *rqstp)
{
    ...
    *result = cudaMemset2DAsync(memory_mg_get(&rm_memory, (void *)devPtr),
...
bool_t cuda_memset_3d_1_svc(size_t pitch, ptr devPtr, size_t xsize,
                            size_t ysize, int value, size_t depth,
                            size_t height, size_t width, int *result,
                            struct svc_req *rqstp)
{
...
bool_t cuda_memset_3d_async_1_svc(size_t pitch, ptr devPtr, size_t xsize,
                                  size_t ysize, int value, size_t depth,
                                  size_t height, size_t width, ptr stream,
                                  int *result, struct svc_req *rqstp)
{
...
bool_t cuda_memset_async_1_svc(ptr devPtr, int value, size_t count, ptr stream,
                               int *result, struct svc_req *rqstp)
{
    ...
    *result =
        cudaMemsetAsync(memory_mg_get(&rm_memory, (void *)devPtr), value, count,
                        resource_mg_get(&rm_streams, (void *)stream));
    RECORD_RESULT(integer, *result);
    return 1;
}
/* cudaMipmappedArrayGetSparseProperties ( cudaArraySparseProperties*
 * sparseProperties, cudaMipmappedArray_t mipmap ) is not implemented */
/* make_cudaExtent ( size_t w, size_t h, size_t d ) should be implemented on the
 * client side */
/* make_cudaPitchedPtr ( void* d, size_t p, size_t xsz, size_t ysz ) should be
...
bool_t cuda_device_can_access_peer_1_svc(int device, int peerDevice,
                                         int_result *result,
                                         struct svc_req *rqstp)
{
    LOGE(LOG_DEBUG, "cudaDeviceCanAccessPeer");
    result->err =
        cudaDeviceCanAccessPeer(&result->int_result_u.data, device, peerDevice);
    return 1;
}

bool_t cuda_device_disable_peer_access_1_svc(int peerDevice, int *result,
                                             struct svc_req *rqstp)
{
    RECORD_API(int);
...
bool_t cuda_device_enable_peer_access_1_svc(int peerDevice, int flags,
                                            int *result, struct svc_req *rqstp)
{
...
bool_t cuda_register_fat_binary_1_svc(rpc_fatCubin cubin, ptr_result *result,
struct svc_req *rqstp)
{
    struct __fatCubin fat = {.magic = cubin.magic,
                             ...
                             .ptr2  = cubin.ptr2,
                             .zero  = cubin.zero};
    printf("__cudaRegisterFatBinary(magic: %x, seq: %x, text: %lx, data: %lx,
ptr: %lx, ptr2: %lx, zero: %lx\n", fat.magic, fat.seq, fat.text, fat.data,
fat.ptr, fat.ptr2, fat.zero);
    //result->ptr_result_u.ptr = (uint64_t)__cudaRegisterFatBinary(&fat);
    result->err = 0;
    return 1;
}

bool_t cuda_register_function_1_svc(ptr cubinHandle, ptr hostFun, char
*deviceFun, char *deviceName, int *result, struct svc_req * rqstp)
{
    LOGE(LOG_DEBUG, "__cudaRegisterFunction(fatCubinHandle=%p, hostFun=%p,
devFunc=%s, deviceName=%s)\n", (void*)cubinHandle, (void*)hostFun, deviceFun,
deviceName);
   // __cudaRegisterFunction((void*)cubinHandle, (void*)hostFun, deviceFun,
deviceName,                            -1, NULL, NULL, NULL, NULL, NULL);
    *result = 0;
    return 1;
}

bool_t cuda_register_fat_binary_end_1_svc(ptr cubinHandle, int *result, struct
svc_req * rqstp)
{
