...
bool cricket_file_read_mem(const char *path, cricketDataType data_type, const char *suffix, void *data, size_t size)
{
    size_t data_size;
    bool ret;

    ret = cricket_file_read_mem_size(path, data_type, suffix, &data, size, &data_size);
    
    if (ret == false)
        return ret;

    if (data_size != size) {
        fprintf(stderr, "cricket-file: stored size does not match actual size\n");
        return false;
    }
    return ret;
}

bool cricket_file_exists(const char *path, cricketDataType data_type, const char *suffix)
{
...
bool cricket_file_read_mem_size(const char *path, cricketDataType data_type, const char *suffix, void **data, size_t alloc_size, size_t *size)
{
    struct stat path_stat = {0};
    FILE *fp = NULL;
    char *file_name;
    ...
    if (read_dt != data_type) {
        fprintf(stderr, "cricket-file: read dt != given dt. This points to a checkpoint inconsistency! (read %u, given %u)\n", read_dt, data_type);
        goto cleanup;
    }
    ...
    if (*data == NULL) {
        *data = malloc(*size);
        ...
        fprintf(stderr, "cricket-file: allocated size to small. require at least %u bytes.\n", *size);
        goto cleanup;
    }
...
bool cricket_file_store_mem(const char *path, cricketDataType data_type, const char *suffix, void *data, size_t size)
{
    struct stat path_stat = {0};
    FILE *fp = NULL;
    char *file_name;
