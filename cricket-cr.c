...
bool cricket_cr_kernel_name(CUDBGAPI cudbgAPI, uint32_t dev, uint32_t sm, uint32_t wp, const char **name)
{
    CUDBGResult res;
    uint64_t grid_id;
    CUDBGGridInfo info;
...
static bool cricket_cr_gen_suffix (char **suffix, cricketWarpInfo *wi, uint32_t lane)
{
    size_t ret;
    if (lane == CRICKET_CR_NOLANE) {
        ret = asprintf(suffix, "-D%uS%uW%u", wi->dev, wi->sm, wi->warp);
    } else  {
        ret = asprintf(suffix, "-D%uS%uW%uL%u", wi->dev, wi->sm, wi->warp, lane);
    }
    if (ret < 0) {
...
bool cricket_cr_rst_lane(CUDBGAPI cudbgAPI, cricketWarpInfo *wi, uint32_t lane, const char *ckp_dir)
{
    CUDBGResult res;
    ...
    if ((reg_mem = malloc(register_size)) == NULL) {
        fprintf(stderr, "cricket-cr (%d): error during memory allocation of size %d\n", __LINE__, register_size);
        goto cleanup;
    }


    if (!cricket_file_read_mem(ckp_dir, CRICKET_DT_REGISTERS, suffix, reg_mem, register_size)) {
    ...
    if (!cricket_register_rst(cudbgAPI, wi->dev, wi->sm, wi->warp, lane, reg_mem, wi->dev_prop)) {
    ...
    if (!cricket_file_read_mem(ckp_dir, CRICKET_DT_STACK, suffix, stack_mem, wi->stack_size)) {
    ...
    if (!cricket_stack_set_mem(cudbgAPI, wi->dev, wi->sm, wi->warp, lane, stack_mem, stack_loc, wi->stack_size)) {
    ...
    double dt = ((double)((d.tv_sec*1000000+d.tv_usec)-(c.tv_sec*1000000+c.tv_usec)))/1000000.;
    double et = ((double)((e.tv_sec*1000000+e.tv_usec)-(d.tv_sec*1000000+d.tv_usec)))/1000000.;
    double gt = ((double)((g.tv_sec*1000000+g.tv_usec)-(e.tv_sec*1000000+e.tv_usec)))/1000000.;
    double comt = ((double)((g.tv_sec*1000000+g.tv_usec)-(b.tv_sec*1000000+b.tv_usec)))/1000000.;
    printf("lane time:\n\t\tPROFILE readreg: %f s\n\t\tPROFILE setreg: %f s\n\t\tPROFILE readstack: %f s\n\t\tPROFILE setstack: %f s\n\t\tPROFILE lanecomplete: %f s\n",ct,dt,et,gt,comt);
#endif
    ret = true;
 cleanup:
    free(reg_mem);
...
bool cricket_cr_ckp_ssy(CUDBGAPI cudbgAPI, cricketWarpInfo *wi, uint32_t lane, const char *ckp_dir)
{
    uint64_t pc;
    uint64_t virt_pc;
    uint64_t offset;
    ...
    for (offset = 0L; offset <= pc; offset+=0x8) {
        res = cudbgAPI->readCodeMemory(wi->dev, virt_pc-offset, &cur_instr, sizeof(uint64_t));
        if (res != CUDBG_SUCCESS) {
            printf("%d: %s", __LINE__, cudbgGetErrorString(res));
            return false;
        }
...
bool cricket_cr_read_pc(cricketWarpInfo *wi, uint32_t lane, const char *ckp_dir, cricket_callstack *callstack)
{
    CUDBGResult res;
    void *packed = NULL;
    size_t packed_size;
    ...
    if (!cricket_file_read_mem_size(ckp_dir, CRICKET_DT_PC, suffix, &packed, 0, &packed_size)) {
        fprintf(stderr, "cricket-cr: error while reading pc memory\n");
        goto cleanup;
    }

    if (packed_size < offset) {
        fprintf(stderr, "cricket-cr: pc checkpoint file is corrupt: no callstack_size\n");
        goto cleanup;
    }
    ...
    if (packed_size < offset) {
        fprintf(stderr, "cricket-cr: pc checkpoint file is corrupt: too few pc_data entries\n");
        goto cleanup;
    }
    ...
    if (packed_size < offset+pc_data[callstack_size-1].str_offset+1) {
        fprintf(stderr, "cricket-cr: pc checkpoint file is corrupt: string data to short\n");
        goto cleanup;
    }
...
// One specifc warp
bool cricket_cr_rst_subcall(CUDBGAPI cudbgAPI, cricketWarpInfo *wi, cricket_callstack *callstack, int c_level, cricket_jmptable_entry *cal, size_t cal_num, uint64_t *cur_address)
{
    ...
    if (!cricket_elf_get_jmptable_addr(cal, cal_num, callstack->pc[c_level].relative, &jmptbl_address)) {
    ...
    for (uint32_t lane=0; lane != wi->dev_prop->numLanes; lane++) {
        if (lanemask & (1LU<<lane)) {
            res = cudbgAPI->writeRegister(wi->dev, wi->sm, wi->warp, lane, CRICKET_JMX_ADDR_REG, ((uint32_t)(jmptbl_address-*cur_address-0x8)));
            if (res != CUDBG_SUCCESS) {
...
// One specifc warp
bool cricket_cr_rst_ssy(CUDBGAPI cudbgAPI, cricketWarpInfo *wi, cricket_callstack *callstack, int c_level, cricket_jmptable_entry *ssy, size_t ssy_num, uint64_t *cur_address)
{
    ...
    if (!cricket_elf_pc_info(callstack->function_names[c_level], callstack->pc[c_level].relative, &relative_ssy, NULL)) {
    ...
    if (relative_ssy >= cmp_address) {
        if (!cricket_elf_get_jmptable_addr(ssy, ssy_num, relative_ssy, &jmptbl_address)) {
            fprintf(stderr, "error getting jmptable adress\n");
            goto error;
        }
...
bool cricket_cr_rst_pc(CUDBGAPI cudbgAPI, cricketWarpInfo *wi, uint32_t lane_param, cricket_callstack *callstack)
{
    CUDBGResult res;
    bool ret = false;
    uint32_t lane;
    ...
    if (callstack->callstack_size == 1) {
        ...
        if (callstack->active_lanes != callstack->valid_lanes) {
            fprintf(stderr, "cricket-cr: divergent threads in call levels > 1 are not allowed!\n");
            return false;
        }
        ...
        /*if (callstack->active_lanes & (1<<lane)) {
            ...
            address = callstack->pc[0].absolute-callstack->pc[0].relative+0x1150;
        }*/
    } else {
        fprintf(stderr, "cricket-cr: callstacks greater than 2 cannot be restored\n");
        return false;
    }
    ...
    res = cudbgAPI->writePredicates(wi->dev, wi->sm, wi->warp, lane, 1, &predicate);
    if (res != CUDBG_SUCCESS) {
        fprintf(stderr, "cricket-cr (%d): %s\n", __LINE__, cudbgGetErrorString(res));
        return false;
    }
    ...
    do {
        res = cudbgAPI->writeRegister(wi->dev, wi->sm, wi->warp, lane, CRICKET_JMX_ADDR_REG, ((uint32_t)address));

        if (res != CUDBG_SUCCESS) {
            fprintf(stderr, "cricket-cr (%d) D%uS%uW%uL%u: %s, retrying...\n", __LINE__, wi->dev, wi->sm, wi->warp, lane, cudbgGetErrorString(res));
        }
    } while(res != CUDBG_SUCCESS);

    /*res = cudbgAPI->writeRegister(wi->dev, wi->sm, wi->warp, lane, CRICKET_JMX_ADDR_REG+1, (uint32_t)0x0);
    if (res != CUDBG_SUCCESS) {
        fprintf(stderr, "cricket-cr (%d): %s\n", __LINE__, cudbgGetErrorString(res));
        goto cleanup;
    }*/
...
bool cricket_cr_callstack(CUDBGAPI cudbgAPI, cricketWarpInfo *wi, uint32_t lane_param, cricket_callstack *callstack)
{
    uint32_t base_addr;
    uint64_t call_instr;
    ...
    res = cudbgAPI->readCallDepth(wi->dev, wi->sm, wi->warp, lane ,&callstack_size);
    if (res != CUDBG_SUCCESS) {
        fprintf(stderr, "cricket-cr (%d): %s\n", __LINE__, cudbgGetErrorString(res));
        return false;
    }
    ...
    res = cudbgAPI->readVirtualPC(wi->dev, wi->sm, wi->warp, lane, &pc_data[0].virt);
    if (res != CUDBG_SUCCESS) {
        fprintf(stderr, "cricket-cr (%d): %s\n", __LINE__, cudbgGetErrorString(res));
        goto cleanup;
    }
    ...
    for (i=1; i < callstack_size; ++i) {
        ...
        res = cudbgAPI->readReturnAddress(wi->dev, wi->sm, wi->warp, lane, i-1, &pc_data[i].relative);
        if (res != CUDBG_SUCCESS) {
            fprintf(stderr, "cricket-cr (%d): %s\n", __LINE__, cudbgGetErrorString(res));
            goto cleanup;
        }

        res = cudbgAPI->readVirtualReturnAddress(wi->dev, wi->sm, wi->warp, lane, i-1, &pc_data[i].virt);
        if (res != CUDBG_SUCCESS) {
            fprintf(stderr, "cricket-cr (%d): %s\n", __LINE__, cudbgGetErrorString(res));
            goto cleanup;
        }
        ...
        res = cudbgAPI->readCodeMemory(wi->dev, pc_data[i].virt-sizeof(uint64_t), &call_instr, sizeof(uint64_t));
        if (res != CUDBG_SUCCESS) {
            fprintf(stderr, "cricket-cr (%d): %s\n", __LINE__, cudbgGetErrorString(res));
            goto cleanup;
        }
...
bool cricket_cr_ckp_pc(CUDBGAPI cudbgAPI, cricketWarpInfo *wi, uint32_t lane_param, const char *ckp_dir, cricket_callstack *callstack)
{
    char *suffix;
    void *packed = NULL;
    size_t packed_size;
    ...
    str_size = callstack->pc[callstack->callstack_size-1].str_offset + strlen(callstack->function_names[callstack->callstack_size-1])+1;

    packed_size = 3*sizeof(uint32_t)+callstack->callstack_size*sizeof(cricket_pc_data)+str_size;
    ...
    offset+=sizeof(uint32_t);
    memcpy(packed+offset, callstack->pc, callstack->callstack_size*sizeof(cricket_pc_data));
    offset+=callstack->callstack_size*sizeof(cricket_pc_data);
    for (i = 0; i < callstack->callstack_size; ++i) {
        strcpy(packed+offset+callstack->pc[i].str_offset, callstack->function_names[i]);
    }
    ...
    for (i=0; i < callstack->callstack_size; ++i) {
        printf("relative: %lx, absolute: %lx, str_offset: %lx, function: %s\n", callstack->pc[i].relative,
                                                                                callstack->pc[i].absolute,
                                                                                callstack->pc[i].str_offset,
    ...
    if (!cricket_file_store_mem(ckp_dir, CRICKET_DT_PC, suffix, packed, packed_size)) {
...
/*
bool cricket_cr_ckp_pc(CUDBGAPI cudbgAPI, cricketWarpInfo *wi, uint32_t lane_param, const char *ckp_dir)
{
    uint32_t base_addr;
    uint64_t call_instr;
    cricket_pc_data *pc_data = NULL;
    ...
    for (i=1; i < callstack_size; ++i) {
        ...
        res = cudbgAPI->readVirtualReturnAddress(wi->dev, wi->sm, wi->warp, lane, i-1, &virt_pc);
        if (res != CUDBG_SUCCESS) {
            fprintf(stderr, "cricket-cr (%d): %s\n", __LINE__, cudbgGetErrorString(res));
            goto cleanup;
        }
        ...
        res = cudbgAPI->readCodeMemory(wi->dev, virt_pc-sizeof(uint64_t), &call_instr, sizeof(uint64_t));
        if (res != CUDBG_SUCCESS) {
            fprintf(stderr, "cricket-cr (%d): %s\n", __LINE__, cudbgGetErrorString(res));
            goto cleanup;
        }
    ...
    packed_size = 3*sizeof(uint32_t)+callstack_size*sizeof(cricket_pc_data)+str_offset;
    if ((packed = malloc(packed_size)) == NULL) {
        goto cleanup;
    }
...
bool cricket_cr_make_checkpointable(CUDBGAPI cudbgAPI, cricketWarpInfo *wi, cricket_function_info *fi, size_t fi_num, cricket_callstack *callstack)
{
    CUDBGResult res;
    ...
    for (i = callstack->callstack_size -1; i+1 > 0; --i) {
        if (!cricket_elf_get_fun_info(fi, fi_num, callstack->function_names[i], &the_fi)) {
        ...
        printf ("function \"%s\" has %sroom (%d slots)\n", callstack->function_names[i], (the_fi->has_room ? "" : "no "), the_fi->room);
        if (i == callstack->callstack_size-1 && the_fi->room == 0) {
            fprintf(stderr, "cricket-cr: There is no room in the top level function (i.e. the kernel). This kernel can thus never be restored!\n");
            goto cleanup;
        }
        if (!the_fi->has_room) {
            printf("function %s does not have enough room. Subcalls and synchronization points in this function cannot be restored\n", callstack->function_names[i]);
            if (i > 1) {
                printf ("no room in %s. going up to %lx (+%lx)...\n", callstack->function_names[i], callstack->pc[i].virt, callstack->pc[i].relative);
                res = cudbgAPI->resumeWarpsUntilPC (wi->dev, wi->sm, (0x1 << wi->warp), callstack->pc[i].virt+0x8 );
                if (res != CUDBG_SUCCESS) {
            ...
            if (!cricket_elf_pc_info(callstack->function_names[i], callstack->pc[i].relative, &rel_ssy, &rel_pbk)) {
                fprintf(stderr, "cricket: pc info failed\n");
                goto cleanup;
            }
            printf("ssy: %lx > relative: %lx ?\n", rel_ssy, callstack->pc[i].relative);
            if (rel_ssy > callstack->pc[i].relative) {
                printf("joining to pc %lx\n", callstack->pc[i].virt-callstack->pc[i].relative+rel_ssy);

                res = cudbgAPI->resumeWarpsUntilPC (wi->dev, wi->sm, (0x1 << wi->warp), callstack->pc[i].virt-callstack->pc[i].relative+rel_ssy );
                if (res != CUDBG_SUCCESS) {
                    printf("cuda-cr (%d): resumeWarpsUntilPC: ", __LINE__);
                    printf("Cuda Error: \"%s\"\n", cudbgGetErrorString(res));
                    goto cleanup;
            ...
            if (rel_pbk > callstack->pc[i].relative) {
                printf("breaking to pc %lx\n", callstack->pc[i].virt-callstack->pc[i].relative+rel_pbk);

                res = cudbgAPI->resumeWarpsUntilPC (wi->dev, wi->sm, (0x1 << wi->warp), callstack->pc[i].virt-callstack->pc[i].relative+rel_pbk );
                if (res != CUDBG_SUCCESS) {
                    printf("cuda-cr (%d): resumeWarpsUntilPC: ", __LINE__);
                    printf("Cuda Error: \"%s\"\n", cudbgGetErrorString(res));
                    goto cleanup;
            ...
            if (joined || stepup) {
                ...
                if (!cricket_cr_callstack(cudbgAPI, wi, CRICKET_CR_NOLANE, callstack)) {
                    fprintf(stderr, "cricket-cr: failed to get callstack\n");
                    goto cleanup;
                }
                printf("new callstack size %d\n", callstack->callstack_size);
                if (callstack->callstack_size != callstack_bku-i) {
                    fprintf(stderr, "cricket-cr: new callstack has wrong size\n");
                    goto cleanup;
                }
                if (joined && callstack->valid_lanes != callstack->active_lanes) {
                    fprintf(stderr, "cricket-cr: joning failed, threads still divergent @ rel PC %lx\n", callstack->pc[0].relative);
                    goto cleanup;
                }
            }
            break;
...
/*
bool cricket_cr_join_threads(CUDBGAPI cudbgAPI, uint32_t dev, uint32_t sm, uint32_t wp)
{
    CUDBGResult res;
    uint64_t rel_pc;
    uint64_t virt_pc;
    ...
    if (rel_ssy == 0 || rel_ssy < rel_pc) {
        fprintf(stderr, "cricket-cr: no applicable ssy instruction found. Threads may be diverged due to something else (propably unsupported)\n");
        goto cleanup;
    }
    res = cudbgAPI->resumeWarpsUntilPC (dev, sm, (0x1 << wp), virt_pc-rel_pc+rel_ssy );
    if (res != CUDBG_SUCCESS) {
        printf("cuda-cr (%d): resumeWarpsUntilPC: ", __LINE__);
        printf("Cuda Error: \"%s\"\n", cudbgGetErrorString(res));
        goto cleanup;
    ...
    if (valid_lanemask != active_lanemask) {
        fprintf(stderr, "cricket-cr: joining threads failed :( (active: %x, valid: %x)\n", active_lanemask, valid_lanemask);
        goto cleanup;
    }
...
/*#define _CRICKET_DEBUG_CR_ 1
bool cricket_cr_ckp_pc(CUDBGAPI cudbgAPI, cricketWarpInfo *wi, uint32_t lane, const char *ckp_dir)
{
    ...
    res = cudbgAPI->readCallDepth(wi->dev, wi->sm, wi->warp, lane ,&pc_data.call_depth);
    if (res != CUDBG_SUCCESS) {
        fprintf(stderr, "cricket-cr (%d): %s\n", __LINE__, cudbgGetErrorString(res));
        return false;
    }
    ...
    if (pc_data.call_depth != 1) {
        fprintf(stderr, "cricket-cr: call-depth unequal 1 currently not supported\n");
        return false;
    }

    res = cudbgAPI->readVirtualReturnAddress(wi->dev, wi->sm, wi->warp, lane, pc_data.call_depth-1, &virt_ret_addr);
    if (res != CUDBG_SUCCESS) {
        fprintf(stderr, "cricket-cr (%d): %s\n", __LINE__, cudbgGetErrorString(res));
        return false;
    }

    res = cudbgAPI->readReturnAddress(wi->dev, wi->sm, wi->warp, lane, pc_data.call_depth-1, &pc_data.relative_ret_addr);
    if (res != CUDBG_SUCCESS) {
        fprintf(stderr, "cricket-cr (%d): %s\n", __LINE__, cudbgGetErrorString(res));
        return false;
    }

    res = cudbgAPI->readCodeMemory(wi->dev, virt_ret_addr-sizeof(uint64_t), &call_instr, sizeof(uint64_t));
    if (res != CUDBG_SUCCESS) {
        fprintf(stderr, "cricket-cr (%d): %s\n", __LINE__, cudbgGetErrorString(res));
        return false;
    }
    ...
    res = cudbgAPI->readPC(wi->dev, wi->sm, wi->warp, lane, &pc_data.relative_addr);
    if (res != CUDBG_SUCCESS) {
        fprintf(stderr, "cricket-cr (%d): %s\n", __LINE__, cudbgGetErrorString(res));
        return false;
    }
    ...
    if (!cricket_file_store_mem(ckp_dir, CRICKET_DT_PC, suffix, &pc_data, sizeof(struct cricket_pc_data))) {
        fprintf(stderr, "cricket-cr: error writing pc\n");
        free(suffix);
        return false;
    }
...
#define CRICKET_PROFILE 1
bool cricket_cr_ckp_lane(CUDBGAPI cudbgAPI, cricketWarpInfo *wi, uint32_t lane, const char *ckp_dir)
{
    CUDBGResult res;
    size_t register_size;
    void *mem = NULL;
    ...
    if (!cricket_stack_get_mem(cudbgAPI, wi->dev, wi->sm, wi->warp, lane, mem, wi->stack_size)) {
        fprintf(stderr, "cricket-cr: error while retrieving stack memory\n");
        goto cleanup;
    }
    ...
    if (!cricket_file_store_mem(ckp_dir, CRICKET_DT_STACK, suffix, mem, wi->stack_size)) {
    ...
    if (!cricket_register_ckp(cudbgAPI, wi->dev, wi->sm, wi->warp, lane, mem, wi->dev_prop)) {
    ...
    if (!cricket_file_store_mem(ckp_dir, CRICKET_DT_REGISTERS, suffix, mem, register_size)) {
    ...
    double gt = ((double)((g.tv_sec*1000000+g.tv_usec)-(e.tv_sec*1000000+e.tv_usec)))/1000000.;
    double comt = ((double)((g.tv_sec*1000000+g.tv_usec)-(b.tv_sec*1000000+b.tv_usec)))/1000000.;
    printf("lane time:\n\t\tPROFILE getstack: %f s\n\t\tPROFILE storestack: %f s\n\t\tPROFILE getreg: %f s\n\t\tPROFILE storereg: %f s\n\t\tPROFILE lanecomplete: %f s\n",ct,dt,et,gt,comt);
#endif
...
bool cricket_cr_rst_params(CUDBGAPI cudbgAPI, const char *ckp_dir, cricket_elf_info *elf_info, uint32_t dev, uint32_t sm, uint32_t warp)
{
    CUDBGResult res;
     ...
     * TODO: use first valid warp, because warp 0 may not be in use (is that possible?)
     */
    if ((param_mem = malloc(elf_info->param_size)) == NULL)
        return false;

    if (!cricket_file_read_mem(ckp_dir, CRICKET_DT_PARAM, NULL, param_mem, elf_info->param_size)) {
    ...
    res = cudbgAPI->writeParamMemory(dev, sm, warp, (uint64_t)elf_info->param_addr, param_mem, (uint32_t)elf_info->param_size);
    if (res != CUDBG_SUCCESS) {
        fprintf(stderr, "cricket-cr (%d):%s", __LINE__, cudbgGetErrorString(res));
        goto cleanup;
    }
    for (int i=0; i != elf_info->param_num; ++i) {
        ...
        param_data = NULL;
        if (!cricket_file_read_mem_size(ckp_dir, CRICKET_DT_HEAP, heap_suffix, &param_data, 0, &heapsize)) {
            printf("cricket error while reading heap param data\n");
            goto cleanup;
        }

#ifdef _CRICKET_DEBUG_CR_
        printf("heap param %u: %llx (%u)\n", i, *(void**)(param_mem+elf_info->params[i].offset), heapsize);
        ...
        res = cudbgAPI->writeGlobalMemory(*(uint64_t*)(param_mem+elf_info->params[i].offset), param_data, heapsize);
        if (res != CUDBG_SUCCESS) {
            fprintf(stderr, "cricket-cr (%d): %s\n", __LINE__, cudbgGetErrorString(res));
            goto cleanup;
        }
...
bool cricket_cr_ckp_params(CUDBGAPI cudbgAPI, const char *ckp_dir, cricket_elf_info *elf_info, uint32_t dev, uint32_t sm, uint32_t warp)
{
    CUDBGResult res;
    uint8_t *param_mem = NULL;
    void *param_data = NULL;
    ...
    res = cudbgAPI->readParamMemory(dev, sm, warp, (uint64_t)elf_info->param_addr, param_mem, (uint32_t)elf_info->param_size);
    if (res != CUDBG_SUCCESS) {
        fprintf(stderr, "cricket-cr (%d):%s", __LINE__, cudbgGetErrorString(res));
        goto cleanup;
    }
    ...
    if (!cricket_file_store_mem(ckp_dir, CRICKET_DT_PARAM, NULL, param_mem, elf_info->param_size)) {
    ...
    for (int i=0; i != elf_info->param_num; ++i) {
        ...
        if (!cricket_heap_memreg_size(*(void**)(param_mem+elf_info->params[i].offset), &heapsize)) {
            printf("cricket-heap: param %u is a 64 bit parameter but does not point to an allocated region or an error occured\n", i);
            continue;
        }
        ...
        res = cudbgAPI->readGlobalMemory(*(uint64_t*)(param_mem+elf_info->params[i].offset), param_data, heapsize);
        if (res != CUDBG_SUCCESS) {
            fprintf(stderr, "cricket-cr (%d): %s\n", __LINE__, cudbgGetErrorString(res));
            goto cleanup;
        }
        ...
        sprintf(heap_suffix, "-P%u", elf_info->params[i].index);
        if (!cricket_file_store_mem(ckp_dir, CRICKET_DT_HEAP, heap_suffix, param_data, heapsize)) {
...
bool cricket_cr_ckp_shared(CUDBGAPI cudbgAPI, const char *ckp_dir, cricket_elf_info *elf_info, uint32_t dev, uint32_t sm, uint32_t warp)
{
    CUDBGResult res;
    ...
    res = cudbgAPI->readSharedMemory(dev, sm, warp, 0x0LLU, shared_mem, (uint32_t)elf_info->shared_size);
    if (res != CUDBG_SUCCESS) {
        fprintf(stderr, "cricket-cr (%d):%s\n", __LINE__, cudbgGetErrorString(res));
        goto cleanup;
    }
    ...
    if (!cricket_file_store_mem(ckp_dir, CRICKET_DT_SHARED, warp_suffix, shared_mem, elf_info->shared_size)) {
        printf("error writing param\n");
        goto cleanup;
    }
    ret = true;
...
bool cricket_cr_rst_shared(CUDBGAPI cudbgAPI, const char *ckp_dir, cricket_elf_info *elf_info, uint32_t dev, uint32_t sm, uint32_t warp)
{
    CUDBGResult res;
    uint8_t *shared_mem = NULL;
    char warp_suffix[16];
    ...
    if (!cricket_file_read_mem(ckp_dir, CRICKET_DT_SHARED, warp_suffix, shared_mem, elf_info->shared_size)) {
    ...
    res = cudbgAPI->writeSharedMemory(dev, sm, warp, 0x0LLU, shared_mem, (uint32_t)elf_info->shared_size);
    if (res != CUDBG_SUCCESS) {
        fprintf(stderr, "cricket-cr (%d):%s", __LINE__, cudbgGetErrorString(res));
        goto cleanup;
    }
...
bool cricket_cr_rst_globals(CUDBGAPI cudbgAPI, const char *ckp_dir)
{
    ...
    if (!cricket_file_read_mem(ckp_dir, CRICKET_DT_GLOBALS, NULL, globals_mem, globals_mem_size)) {
    ...
    for (i=0; i < globals_num; ++i) {
        res = cudbgAPI->writeGlobalMemory(globals[i].address, globals_mem+offset, globals[i].size);
        if (res != CUDBG_SUCCESS) {
            fprintf(stderr, "cricket-cr (%d):%s", __LINE__, cudbgGetErrorString(res));
            goto cleanup;
        }
    ...
    for (i=0; i != globals_num; ++i) {
        ...
        globals_data = NULL;
        if (!cricket_file_read_mem_size(ckp_dir, CRICKET_DT_HEAP, heap_suffix, &globals_data, 0, &heapsize)) {
...
#ifdef _CRICKET_DEBUG_CR_
        printf("heap global %u: %llx (%u)\n", i, *(void**)(globals_mem+offset), heapsize);
        ...
        res = cudbgAPI->writeGlobalMemory(*(uint64_t*)(globals_mem+offset), globals_data, heapsize);
        if (res != CUDBG_SUCCESS) {
            fprintf(stderr, "cricket-cr (%d): %s\n", __LINE__, cudbgGetErrorString(res));
            goto cleanup;
        }
...
bool cricket_cr_ckp_globals(CUDBGAPI cudbgAPI, const char *ckp_dir)
{
    ...
    for (i=0; i < globals_num; ++i) {
        res = cudbgAPI->readGlobalMemory(globals[i].address, globals_mem+offset, globals[i].size);
        if (res != CUDBG_SUCCESS) {
            fprintf(stderr, "cricket-cr (%d):%s", __LINE__, cudbgGetErrorString(res));
            goto cleanup;
        }
    ...
    if (!cricket_file_store_mem(ckp_dir, CRICKET_DT_GLOBALS, NULL, globals_mem, globals_mem_size)) {
    ...
    for (i=0; i != globals_num; ++i, offset += globals[i].size) {
        ...
        if (!cricket_heap_memreg_size(*(void**)(globals_mem+offset), &heapsize)) {
            printf("cricket-heap: global variable %s has a size of 64 bit but does not point to an allocated region or an error occured\n", globals[i].symbol);
            continue;
        }
        printf("heap param %u: %llx (%u)\n", i, *(void**)(globals_mem+offset), heapsize);
        ...
        res = cudbgAPI->readGlobalMemory(*(uint64_t*)(globals_mem+offset), globals_data, heapsize);
        if (res != CUDBG_SUCCESS) {
            fprintf(stderr, "cricket-cr (%d): %s\n", __LINE__, cudbgGetErrorString(res));
            goto cleanup;
        }
...
#endif
        asprintf(&heap_suffix, "-G%s", globals[i].symbol);
        if (!cricket_file_store_mem(ckp_dir, CRICKET_DT_HEAP, heap_suffix, globals_data, heapsize)) {
            printf("cricket error while writing param heap\n");
        }
        free(heap_suffix);
        heap_suffix = NULL;
